...
describe('Transfer Limit Policies', () => {
  ...
  describe('Policy Comparison', () => {
    ...
    it('should have corporate limits higher than pyme limits', () => {
      expect(corporatePolicy.getDailyLimit()).toBeGreaterThan(pymePolicy.getDailyLimit());
      expect(corporatePolicy.getMonthlyLimit()).toBeGreaterThan(pymePolicy.getMonthlyLimit());
    });
    ...
    it('should allow same transfer for corporate but reject for pyme', () => {
      ...
      const pymeResult = pymePolicy.isTransferAllowed(amount, dailyUsed, monthlyUsed);
      const corporateResult = corporatePolicy.isTransferAllowed(amount, dailyUsed, monthlyUsed);
